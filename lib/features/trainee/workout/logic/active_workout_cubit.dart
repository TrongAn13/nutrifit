...
class ActiveWorkoutState {
  ...
  /// The planId of the plan this workout belongs to.
  final String? planId;
  ...
  /// Whether the workout screen is minimized to a mini-player bar.
  final bool isMinimized;

  /// Saved phase index when minimized (0=prepare, 1=exercise, 2=rest).
  final int minimizedPhaseIndex;

  /// Saved active timeline index when minimized.
  final int minimizedTimelineIndex;

  /// Saved rest remaining when minimized.
  final int minimizedRestRemaining;

  /// Saved prepare countdown when minimized.
  final int minimizedPrepareSeconds;
  ...
  const ActiveWorkoutState({
    ...
    this.minimizedPhaseIndex = 0,
    this.minimizedTimelineIndex = 0,
  ...
  ActiveWorkoutState copyWith({
    ...
    bool? isFinished,
    bool? isMinimized,
    int? minimizedPhaseIndex,
    int? minimizedTimelineIndex,
    ...
    return ActiveWorkoutState(
      ...
      minimizedPhaseIndex: minimizedPhaseIndex ?? this.minimizedPhaseIndex,
      minimizedTimelineIndex: minimizedTimelineIndex ?? this.minimizedTimelineIndex,
...
class ActiveWorkoutCubit extends Cubit<ActiveWorkoutState> {
  ...
  /// Called when the trainee taps the workout card on the dashboard.
  /// If a session is already active, this call is ignored.
  void loadRoutine(RoutineModel routine, {String? planId}) {
  ...
  /// Begins the workout timer. Called when the trainee presses "Bắt đầu tập luyện".
  void beginWorkout() {
  ...
  void addSet(int exerciseIndex) {
    final updated = Map<int, List<SetData>>.from(state.setsData);
    final sets = List<SetData>.from(updated[exerciseIndex]!);
    final baselineRep = sets.isNotEmpty ? sets.last.reps : 0;
    final baselineWeight = sets.isNotEmpty ? sets.last.weight : 0.0;
    
    sets.add(SetData(weight: baselineWeight, reps: baselineRep, isCompleted: false));
    updated[exerciseIndex] = sets;
    emit(state.copyWith(setsData: updated));
  }
  ...
  void completeAllSets(int exerciseIndex) {
    ...
    for (int i = 0; i < sets.length; i++) {
      sets[i] = sets[i].copyWith(isCompleted: true);
    }
    updated[exerciseIndex] = sets;
    emit(state.copyWith(setsData: updated));
    _startRestTimer();
  ...
  /// Minimize the workout screen to a mini-player bar.
  /// Saves the current phase and timeline position for restoration.
  void minimizeWorkout({
    required int phaseIndex,
    required int timelineIndex,
    required int restRemaining,
    required int prepareSeconds,
    required int exerciseElapsed,
  }) {
    emit(state.copyWith(
      isMinimized: true,
      minimizedPhaseIndex: phaseIndex,
      minimizedTimelineIndex: timelineIndex,
      minimizedRestRemaining: restRemaining,
      minimizedPrepareSeconds: prepareSeconds,
      minimizedExerciseElapsed: exerciseElapsed,
    ));
  }

  /// Restore the workout screen from the mini-player.
  void restoreWorkout() {
