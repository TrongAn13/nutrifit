...
/// A single logged set recorded by the trainee.
@immutable
class LoggedSet {
  final int setNumber;
  final double weight;
...
class ActiveExerciseCubit extends Cubit<ActiveExerciseState> {
  ...
  void logSet(double weight, int reps) {
    ...
    // Only update if it's within the original target bounds, or if the trainee added more sets, WorkoutCubit handles it
    try {
